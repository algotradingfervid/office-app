...
func TestLimitChecks(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "half days that join count", existing: []limitRow{{"E001", pend, "CL", "2026-10-08", "second_half", "2026-10-09", "full", 1.5, false}},
			req: req("CL", "2026-10-12", "2026-10-12", 1), message: consec},
		{name: "afternoon after a morning on the same date joins",
			existing: []limitRow{{"E001", pend, "CL", "2026-10-08", "full", "2026-10-09", "first_half", 1.5, false}},
			req:      limitRow{"E001", pend, "CL", "2026-10-09", "second_half", "2026-10-12", "full", 1.5, false}, message: consec},
		{name: "morning before an afternoon on the same date joins",
			existing: []limitRow{{"E001", pend, "CL", "2026-10-09", "second_half", "2026-10-12", "full", 1.5, false}},
			req:      limitRow{"E001", pend, "CL", "2026-10-08", "full", "2026-10-09", "first_half", 1.5, false}, message: consec},
		{name: "an overlapping request is not adjacent (DateChecks refuses overlaps)", existing: []limitRow{req("CL", "2026-10-12", "2026-10-12", 1)},
			req: limitRow{"E001", pend, "CL", "2026-10-12", "second_half", "2026-10-13", "full", 1.5, false}},
		// existing rows saved out of date order: the walk must still meet the nearest request first
		{name: "chain before adds up", existing: []limitRow{req("PL", "2026-10-09", "2026-10-09", 1), req("PL", "2026-10-07", "2026-10-08", 2)},
			req: req("PL", "2026-10-12", "2026-10-14", 3), message: pl},
		{name: "chain after adds up", existing: []limitRow{req("PL", "2026-10-12", "2026-10-14", 3), req("PL", "2026-10-09", "2026-10-09", 1)},
